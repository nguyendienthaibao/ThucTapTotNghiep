...
recruitmentRouter.post("/searchRecruitment", async (req, res) => {
  try {
    const { searchTerm = "", skip = 0 } = req.body;
    ...
    const query = {
      ...
      ...(keyword && {
        $or: [
          ...
          { "career.name": { $regex: keyword, $options: "i" } },
        ],
      }),
    };

    const result = await Recruitment.find(query)
      .populate("city", "name")
      .populate("career", "name")
      .sort({ createdAt: -1 })
      .skip(skip)
